          ...
          searchResults.map((result: any, index: number) => (
            <View key={index} style={{ marginBottom: 20 }}>
              ...
              >
                {result.image_1 && (
                  <Image
                    source={{ uri: `https://sbsapp.com.ng/${result.image_1}` }}
                    style={styles.image}
                  />
                )}
              </View>
              ...
              >
                {result.image_2 && (
                  <Image
                    source={{ uri: `https://sbsapp.com.ng/${result.image_2}` }}
                    style={styles.image}
                  />
                )}
              </View>
              ...
              <View style={{ alignItems: "center", justifyContent: "center" }}>
                {result.image_3 && (
                  <Image
                    source={{ uri: `https://sbsapp.com.ng/${result.image_3}` }}
                    style={styles.image}
                  />
                )}
                {result.image_4 && (
                  <Image
                    source={{ uri: `https://sbsapp.com.ng/${result.image_4}` }}
                    style={styles.image}
                  />
                )}
                {result.image_5 && (
                  <Image
                    source={{ uri: `https://sbsapp.com.ng/${result.image_5}` }}
                    style={styles.image}
                  />
                )}
              </View>
...
const createMathJaxScript = (content: any) => {
  return `
    ...
    <div id="math"></div>
    <script>
        document.getElementById('math').innerHTML = \`${content.replace(
          /\\/g,
          "\\\\"
        )}\`;
        MathJax.typeset();

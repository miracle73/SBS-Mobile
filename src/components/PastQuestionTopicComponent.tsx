...
import { useGetTopicPastQuestionQuery, useUserActivatedStatusMutation } from '../components/services/userService';
...
const PastQuestionTopicComponent: React.FC<PastQuestionTopicComponentProps> = ({ title, id, free, year, courseName, level }) => {
        ...
        if (!free) {
            try {
                ...
                if (netInfo.isConnected) {
                    const activationStatus = await userActivatedStatus({ phone_imei: phoneImei }).unwrap();
                    console.log("Activation status:", activationStatus);

                    await AsyncStorage.setItem('activationMessage', JSON.stringify(activationStatus.message));

                    const filteredMessage = activationStatus.message.find(msg => msg.level === parseInt(level));
                    ...
                    if (storedMessage) {
                        ...
                        const filteredMessage = parsedMessage.find((msg: any) => msg.level === parseInt(level));
                        if (!filteredMessage || !filteredMessage.is_activated) {
                            Toast.show({
                                type: 'error',
                                text1: 'Error',
        ...
        try {
            ...
            if (netInfo.isConnected) {
                ...
                if (storedContents) {
                    ...
                    if (selectedCourse) {
                        const selectedTopic = selectedCourse.topics.find((topic: any) => topic.topic_title === title);
                        if (selectedTopic && selectedTopic.past_questions) {
                            const offlinePastQuestions = selectedTopic.past_questions.map((pq: any) => ({
                                year: pq.year,
                                content: pq.content,
                                image_1: pq.images[0],
                                image_2: pq.images[1],
            ...
            console.error('Error fetching topic content:', error);
            const errorMessage = (error as any)?.message || 'Failed to fetch topic content.';
            Toast.show({
                type: 'error',
                text1: 'Error',
                text2: errorMessage,
